...
**Known gotcha, not yet fixed:** when a stored preference differs from the system preference, reloading can log a React hydration-mismatch warning in the console (something like *"A tree hydrated but some attributes of the server rendered HTML didn't match the client properties"*, pointing at `data-theme` on `<html>`). This happens because the script sets `data-theme` on the real DOM before React hydrates, but the server-rendered markup — which can't read `localStorage` — never included that attribute, so React's dev-mode checker flags the discrepancy. It's cosmetic, not functional: the theme, label, and `aria-pressed` all stay correct regardless, and React doesn't try to "fix" an attribute it doesn't manage via props. The standard fix is `suppressHydrationWarning` on the `<html>` element, the same approach libraries like `next-themes` use for this exact situation. Found during the `aria-pressed` cycle's regression testing (`agents/runs/2026-07-08-theme-toggle-aria-pressed/review.md`) — it's a known follow-up from the pre-paint script itself, not something that cycle's change caused.
